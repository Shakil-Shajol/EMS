...
namespace EMS.Application.Handlers.Employees
{
    public class EmployeeListQueryHandler : BaseHandler,
        IRequestHandler<EmployeeListQuery, ResponseDetail<List<Employee>>>
    {
        ...
        public async Task<ResponseDetail<List<Employee>>> Handle(EmployeeListQuery request, CancellationToken cancellationToken)
        {
            var responseDetails = new ResponseDetail<List<Employee>>();
